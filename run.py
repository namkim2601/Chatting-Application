...
def manage_user_db():
    '''
        manage_db
        Starts up and re-initialises an SQL database for users
    '''
    database_args = "Users.db"
    Users_db = sql.SQLDatabase(database_args=database_args)
    Users_db.user_database_setup()
    return

def manage_post_db():
    '''
            manage_db
            Starts up and re-initialises an SQL database for posts
        '''

    database_args = "Posts.db"
    Posts_db = sql.SQLDatabase(database_args=database_args)
    Posts_db.post_database_setup()
    return
...
command_list = ["", manage_user_db, manage_post_db, run_server] # Commands start from index 1
